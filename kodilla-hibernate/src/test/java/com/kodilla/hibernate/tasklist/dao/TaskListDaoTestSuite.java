...
public class TaskListDaoTestSuite {
    ...
    public void testFindByListName(){
        ...
        //Given
        TaskList taskList = new TaskList("Aktywacja progrmu","Pobierz klucz aktywacyjny, wpisz i polu aktywacji i zaakceptuj");
    ...
    public void testNamedQueries() {
        ...
        List<Task> durationLongerThanTaska = taskDao.retrieveTasksWithDurationLongerThan(6);

        //Then
        try {
            Assert.assertEquals(1, longTasks.size());
            Assert.assertEquals(1, shortTasks.size());
            Assert.assertEquals(3, enoughTimeTasks.size());
            Assert.assertEquals(2,durationLongerThanTaska.size());
        } finally {
            //CleanUp
            taskListDao.deleteById(id);
        }
